...
public enum ViewCellsBuilder {
    ...
    @inlinable
    public static func buildExpression<C: Collection>(_ expression: C) -> [ViewCell] where C.Element: ViewCellConvertible {
        (expression as? [ViewCell]) ?? LazyArray(expression).map(\.asViewCell)
    }

    @inlinable
    public static func buildExpression<C: Collection>(_ expression: C) -> [ViewCell] where C.Element == any ViewCellConvertible {
